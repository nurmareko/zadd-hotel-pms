...
- **Kalender (Tape Chart)** — default Front Office landing page: occupancy visualization as a room × date grid with unified status colors, room-type groups, unallocated-reservation lanes, a checkout marker, interactive calendar date-range popover (#204), and unified styling for unallocated and out-of-order rows (#205). Clicking an empty cell opens the reservation form with Kalender context prefilled.
- **Daftar Reservasi & Ekspor CSV** — `/app/fo/reservasi/list` provides searchable arrival/departure views with check-in and check-out date filters (#206, #207), expanded status filtering including "Semua Status" (`ALL`) and "No-show" (`NO_SHOW`) (#208, #209), active filter reset, live result count, and role-gated CSV export (`/app/fo/reservasi/export`) using the shared CSV utility (`src/lib/csv.ts`) with formula protection and UTF-8 BOM (#210, #211).
...
- **Shared access and navigation** — all HK users and ADMIN can use the Room Board, assignment, inspection, status override, Daily List print, and room history. The HK supervisor tier is decommissioned: HK and ADMIN have uniform full HK operational access, with no supervisor attribute or tier-based gate. Cleaning still requires assignment to the current operator. Unified HK navigation is Room Board, Laundry, and Lost & Found; Lost & Found is shared by HK, FO, and ADMIN for its page, mutations, and export under #243.
- **HK landing and compatibility routes** — `/app/hk` redirects to `/app/hk/rooms`. `/app/hk/supervisor` is a permanent, query-preserving HTTP 308 compatibility shim to `/app/hk/rooms`, not a separate dashboard. `/app/hk/list` is also a preserved compatibility redirect to the board; neither shim is slated for removal. New links use `/app/hk/rooms`.
...
- **Shared room detail** — `/app/hk/rooms/[id]` provides cleaning context, status/history, and inspection. Only the assigned operator can start/finish cleaning and use its timer. Shared operational access does not bypass the assigned-operator cleaning restriction.
...
- **Manual assignment** — all HK users and ADMIN can assign rooms by date, including bulk assignment by floor/workload; active HK users are eligible assignees. Phase 2 adds inline assignment for the selected operating date. New Phase 2 assignment actions cannot displace an active-cleaning assignee; assigning the same assignee is a no-op. Auto-dispatch remains deferred.
- **Linen circulation (#242)** — implemented at `/app/hk/laundry` for HK and ADMIN, replacing the Laundry placeholder. Unit-based KPIs show cumulative clean units received from `CLEAN` records, dispatched quantities currently `WASHING`, dispatched quantities awaiting processing in `SENT`, and recorded damaged units. Search `q` covers code, vendor, and notes, with status/item/date filters. The dispatch modal offers seven item enum types with Indonesian labels, an integer quantity of at least 1, vendor, and notes; each new `SENT` dispatch receives a sequential `LND-yyMM-####` code. Operators can advance `SENT → WASHING` or receive either `SENT` or `WASHING` once into terminal `CLEAN`. Receipt requires nonnegative integer clean/damaged quantities with `clean + damaged <= sent`, derives lost units as `sent - (clean + damaged)`, records operator/time audit data, and appends receipt notes. Serializable transactions with row locking, authoritative state rechecks, and retries protect transitions and prevent duplicate receipt. All user-facing copy is Indonesian. This is circulation and receipt tracking, not a full stock ledger: no consumption workflow exists, so cumulative clean received must not be presented as current available stock. See the HK-04 contract in [Screen Inventory](./screen_inventory_mvp.md).
...
- **Lost & Found registry (#243)** — `/app/hk/lost-found`, titled **Barang Temuan**, is available to HK, FO, and ADMIN for viewing, creation, claim, disposal/donation, and CSV export, with server-side authorization. Four summary cards show **global, unfiltered record counts**: `Disimpan` (hint `Menunggu Klaim`), `Dikembalikan`, `Dimusnahkan / Dihibahkan`, and `Total Register`. The responsive registry uses a desktop table and mobile/coarse-pointer cards, showing reference/time, item/location, category, finder, status, and actions or recorded resolution; the filtered result count is separate from the summary cards.
- **Found-item creation and labels** — `Catat Barang Temuan` opens a modal for category, required description, `Kamar` or `Area Publik`, room selection when applicable, and location details (required by the public-area form, optional for rooms). `Simpan Barang` creates a stored item with an automatic `LF-YYMM-XXXX` reference using the hotel-local month and a sequence padded to at least four digits. Exact category labels are `Elektronik / Gadget`, `Pakaian / Tekstil`, `Dokumen / Identitas`, `Barang Berharga`, `Aksesoris / Pribadi`, and `Lainnya`. Status labels are `Disimpan` (`UNCLAIMED`), `Dikembalikan` (`RETURNED`), and `Dimusnahkan / Dihibahkan` (`DISPOSED`).
- **Claim and disposal/donation** — `Serahkan ke Tamu` requires `claimantName`; phone, identity number, and handover notes are optional. `Konfirmasi Penyerahan` records the claimant and return staff/time audit. `Musnahkan/Hibah` opens the disposal modal with reasons `Masa simpan berakhir`, `Dihibahkan ke yayasan`, or `Dimusnahkan karena rusak/kedaluwarsa`, plus optional notes; `Konfirmasi Pemusnahan / Hibah` records the reason/notes and disposal staff/time audit. Only `UNCLAIMED → RETURNED` or `UNCLAIMED → DISPOSED` is allowed; both terminal transitions are irreversible. Shared serializable operations lock and reread the item, recheck the active operator/role, and conditionally write once, preventing repeat or conflicting resolution. Historical missing audit fields are not invented.
- **Shared registry filters** — page and export reuse `parseLostFoundFilters` / `buildLostFoundWhere` for `q`, `category`, `status`, `room`, `from`, and `to`. `room` is a trimmed, case-insensitive **room-number substring, not an ID**. The UI exposes `Cari barang`, `Kategori`, `Status`, `Nomor kamar`, `Dari tanggal`, and `Sampai tanggal`, with `Terapkan`, `Atur Ulang`, and `Unduh CSV`. Date bounds include both hotel-local logged days; the download follows the applied URL filters, not unsaved form edits.
- **Lost & Found CSV** — `GET /app/hk/lost-found/export` independently authorizes HK, FO, and ADMIN and downloads all matching records, newest first, as `lost-found-${hotelTodayISO()}.csv`. Shared CSV helpers supply UTF-8 BOM, escaping/formula protection, and `Cache-Control: no-store, max-age=0`. Indonesian headers and shared category/status labels cover registry details, claimant name/phone/identity number, and return/disposal operator, timestamp, and notes. Only custody fields and related room number/operator names are selected; no full guest or user profiles are loaded. Claimant PII requires restricted handling. Export timestamp headers explicitly state **UTC**, with ISO 8601 values; date filtering and the filename use the hotel day. See the full HK-05 contract in [Screen Inventory](./screen_inventory_mvp.md).
- **FO sync** — HK actions revalidate HK screens and Front Office room/tape-chart views.

**Decommissioning and audit continuity:** Preserve existing user identity links, cleaning sessions, inspection attribution, and status/activity logs; removing the tier must not rewrite historical actors or erase audit history. Historical meeting records and archived designs describe earlier decisions, not current authorization. This change does not introduce or remove financial supervisor, waiver, or override permissions.
...
- **Task dialog** — automatic priority is read-only. Each submission creates an immutable log for **actual today in WIB**, not the selected board date, with `oldStatus = newStatus` and a `[TUGAS: category]` note. Optional assignment is also for actual today and is available to HK/ADMIN with active HK assignees. The new actions cannot displace an active-cleaning assignee; same-assignee assignment is a no-op. Task creation does not change room status or append to `Reservation.notes`.
...
Managed by the supervising lecturer. ADMIN manages master data and users and has the same full HK operational access as HK users; assignment and current-state workflow guards still apply.
...
| Reservation list CSV export & shared CSV library (#210, #211) | Front Office | **DONE.** Role-gated `/app/fo/reservasi/export` endpoint downloads filtered reservation records matching active `q`, `status`, `checkIn`, and `checkOut` parameters. Powered by shared `src/lib/csv.ts` providing UTF-8 BOM, spreadsheet formula-injection escaping (`=`, `+`, `-`, `@`), and download headers; reused across HK room board, Lost & Found, and Accounting exports. |
| Reservation list filters & status options (#206–#209) | Front Office | **DONE.** `/app/fo/reservasi/list` includes check-in (`checkIn`) and check-out (`checkOut`) date filters, text search (`q`), and expanded status filtering with default "Aktif" (`CONFIRMED`, `CHECKED_IN`, `CHECKED_OUT`), "Semua Status" (`ALL`), and individual choices including "No-show" (`NO_SHOW`) and "Dibatalkan", plus "Atur Ulang" filter reset and live result count. |
| Tape Chart calendar popover & unified styling (#204, #205) | Front Office | **DONE.** Interactive calendar date-range popover (`DateRangePickerPopover`) on `/app/fo/reservasi/kalender` allows quick date span selection, coupled with unified visual row styling across unallocated reservation lanes and out-of-order (OOO) room blocks. |
